# woodstalker.gd - Woodstalker endless-mode character script
...
var spells_unlocked: Dictionary = {
	"hunters_mark": false,
	"call_pet": false,
	"ensnaring_trap": false,
	"barrage": false,
	"poisoned_shot": false,
	"improved_barrage": false,
}
...
func _ready() -> void:
	...
	# Endless mode: pet only appears once call_pet is unlocked via an
	# ability chest (see _unlock_spell()), unlike player.gd's story mode
	# where it's always present from the start.
	if spells_unlocked.get("call_pet", false):
