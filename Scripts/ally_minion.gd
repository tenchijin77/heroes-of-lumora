# ally_minion.gd - friendly permanent companion for playable summoner-type
# classes (Voidknight's Skeleton, eventually Gravecaller's Wraith).
# Mirrors enemy_pet.gd's movement/attack/respawn-tied-to-owner pattern exactly,
# but targets "monsters" instead of the player's side — enemy_pet.gd can't be
# reused directly since it's hardcoded to attack player/friendly/villagers.
extends CharacterBody2D

const _CONTACT_DIST: float = 15.0

var _owner: Node = null
var _owner_alive: bool = true
...
func _ready() -> void:
	...
	collision_mask = 2048  # Walls only — passes through buildings, respects map boundaries
