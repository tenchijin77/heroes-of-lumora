...
func update_navigation_target() -> void:
	...
	if not navigation_agent:
		push_warning("Villager %s: navigation_agent is null in update_navigation_target" % name)
		return
...
func shout(text: String) -> void:
	# FIXED: Guard against calling when not in tree
	if not is_inside_tree():
		return
...
func reset(type: String = villager_type) -> void:
	...
	if is_inside_tree():
		...
		call_deferred("_deferred_reset_actions")

func _deferred_reset_actions() -> void:
	"""Called when reset() needs to wait for node to be added to tree"""
	if is_inside_tree():
		shout(popup_message)
