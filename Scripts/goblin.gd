...
func _cast():
	...
	if not bullet_pool or not muzzle or not player:
		...
		return
	var dagger = bullet_pool.spawn()
	if dagger:
		dagger.global_position = muzzle.global_position
		dagger.move_direction = muzzle.global_position.direction_to(player.global_position)
		dagger.owner_group = "monsters"  # Ensure owner_group for enemy
